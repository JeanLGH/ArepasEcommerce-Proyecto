...
const Home = () => {
  ...
  useEffect(() => {
    axios.get("http://0.0.0.0:5000/products")
      .then((response) => {
 ...
 const incrementCounter = async (index) => {
  ...
  try {
    const response = await axios.get(
      `http://0.0.0.0:5000/orderitems?id=${orderId}`
    );
    ...
    if (response.data.length > 0) {
      ...
      await axios.put(
        `http://0.0.0.0:5000/orderitems/${orderItem.id}`,
        orderItem
      );
    } else {
      const order = {
      ...
      await axios.post("http://0.0.0.0:5000/orderitems", order); // Crear un nuevo objeto de pedido en el servidor
    }
  } catch (error) {
...
const addItem = async (a, b, c, index) => { // Agrega el parámetro `index`
  ...
  const result = await axios.get("http://0.0.0.0:5000/orderitems");
  if (result.data.length === 0) {
    const order = { id: a, name: b, price: c, qty: quantity };
    axios.post("http://0.0.0.0:5000/orderitems", order);
  } else {
    result.data.forEach((orderItem) => {
      if (a === orderItem.id) {
        ...
        axios.put(`http://0.0.0.0:5000/orderitems/${orderItem.id}`, order);
        isExisting = true;
      }
    });
    if (!isExisting) {
                        ...
                        <Col>
                          <span className="counter-group-card">
                            ...
                            <Button
                              ...
                              onClick={() => addItem(product.id, product.name, product.price, index)} // Agrega `index`
