...
const Payment = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        const response = await axios.get("http://34.132.237.34:5000/customers", {
          params: {
            email: username,
          },
        });
    ...
    const fetchData = async () => {
      try {
        const [orderItemsResponse, productDataResponse] = await Promise.all([
          axios.get("http://34.132.237.34:5000/orderitems"),
  ...
  const sendOrderFinal = async (orderFinalData) => {
    try {
      const response = await axios.post("http://34.132.237.34:5000/orderfinal", orderFinalData);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post("http://34.132.237.34:5000/orders", orderData);
      ...
      const productsResponse = await axios.get('http://34.132.237.34:5000/orderitems');
  ...
  const handleReturnHome = async () => {
    try {
      ...
      for (const orderItem of orderItems) {
        await axios.delete(`http://34.132.237.34:5000/orderitems/${orderItem.id}`);
      }
